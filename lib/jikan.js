...
class JikanAPI {
    ...
    search(type, query, page = null, params = {}) {
        ...
        return loadAPIData(urlBuilder([this.url, 'search', type, query, page, queryParameterBuilder(params)]));
    }
...
/**
 * is for building a string from all given get parameter. In the case that non are given null will retured
 * @param {{}} params               get parameters
 */
var queryParameterBuilder = function(params = null) {
    if(!params) return null; // in the case no parameter are given

    var query = '';
    var first = true;

    for (var key in params) {
        if(first) { query += `?${key}=${params[key]}`; first = false; }
